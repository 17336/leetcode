...
#ifndef HOT100_472ADV_H
#define HOT100_472ADV_H

#endif //HOT100_472ADV_H
...
public:
    vector<string> findAllConcatenatedWordsInADict(vector<string> &words) {
        sort(words.begin(),words.end(),s_less());
        //初始化前缀树
        vector<string> res;//结果数组res
        for (const auto &word: words) {
            if(backtrack(word)) res.push_back(word);
            insert(word);
            set.insert(word);
        }
        return res;
